...
<div class="mx-auto max-w-6xl px-5 pt-28 pb-16 md:pt-44">
    ...
    <div class="mt-6 grid gap-8 lg:grid-cols-2">
        @if ($product->image_url)
            <img src="{{ $product->image_url }}" alt="Foto {{ $product->name }}" class="aspect-square w-full rounded-shell border border-espresso/10 object-cover" loading="lazy">
        @else
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-espresso/70">Status: {{ $state['label'] }}. Harga final dihitung ulang sistem saat checkout.</p>
            ...
            @if ($product->is_active && $product->stock > 0)
                <form method="POST" action="/cart/items" class="mt-8 flex max-w-sm items-center gap-3" aria-label="Tambah ke keranjang">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $product->id }}">
                    <label class="flex-1">
                        <span class="sr-only">Jumlah</span>
                        <input type="number" name="quantity" min="1" max="{{ $product->stock }}" value="{{ old('quantity', 1) }}" required class="min-h-[44px] w-full rounded-pill border border-espresso/20 bg-warm-cream px-5 text-sm" aria-describedby="cart-note">
                    </label>
                    <button type="submit" class="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-pill bg-near-black px-4 py-2 text-sm font-semibold text-warm-cream hover:bg-espresso">Add to Cart</button>
                </form>
                <p id="cart-note" class="mt-2 text-xs text-taupe">Quantity dibatasi stock ({{ $product->stock }}) dan divalidasi server. Butuh login.</p>
            @else
                <p class="mt-8 rounded-core border border-espresso/15 bg-white/40 px-4 py-3 text-sm">Tidak bisa ditambahkan: {{ $product->is_active ? 'stock habis' : 'produk nonaktif' }}.</p>
            @endif
        </div>
    </div>
</div>
